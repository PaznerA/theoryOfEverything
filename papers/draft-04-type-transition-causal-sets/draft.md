...
*(Section provenance: round-10/11 de Sitter extension. Statuses below are quoted verbatim from `core-data/findings.json` F-023 (`supported`) and F-025 (`partial`); nothing here is upgraded beyond those.)*
...
**Honest 2D limit.** The *truncated* (type-II-regularized) SSEE is log/area-flat in **both** geometries (dS late slope $-0.015$, flat $-0.083$): in 2D the truncated entropy does **not** separate the types by magnitude. The II$_1$ vs. II$_\infty$ distinction lives entirely in the **region content** ($N_{\rm total}$, $S_{\rm full}$), not in the truncated entropy. (In a 4D dS patch the truncated $S\sim\sqrt N$ area law would, in principle, separate the types directly — see below.) Separately, the Part-3 maximal-entropic *tracial* probe is an **honest null** at $N\lesssim2500$: the truncated $\langle\varepsilon\rangle$ probe is null by construction (truncation removes exactly the low-$\varepsilon$ tracial modes), and the correct untruncated IR-fraction probe shows no tracial pile-up yet (dS slope $-0.008$, flat $-0.004$); clean resolution would need $\rho\sim10^{3}$–$10^{4}$.
...
> **De Sitter overall: the content discriminator (II$_1$ vs. II$_\infty$) is `supported` in 2D and lifts cleanly to 4D; the truncated-entropy discriminator is the honest 2D null and a real-but-`partial` 4D signal.** The dS patch gives the project's first probe of the *normalizable*-trace (II$_1$) side of the CLPW classification, complementing the II$_\infty$ diamond/slab of §4.1–§4.2.

**Outlook for this section.** Clean truncated-entropy saturation (the strong II$_1$ discriminator in 4D) needs $N$ beyond dense-`eigh` capacity: sparse/iterative eigensolvers at $\rho\sim10^{3}$–$10^{4}$, larger $\ell$, or a fixed-area entangling cut with a denser radial horizon sweep. Also open, and cited here as such, is the $A/4$-cap question (project hypothesis **H5g-2**): whether the saturated dS entropy cap matches the de Sitter horizon area over $4G$ quantitatively — untested, and not claimed.
...
*Data: `core-data/calculations/{sj-vn-type, vn-type-slab-4d, ssee-diamond, ssee-slab-4d, sj-desitter-type, sj-desitter-4d}/results.json`. Provenance notes: `knowledge-base/vypocty/VYPOCET-{04,12,13,16,19,21}*.md`. The de Sitter section (§4.3) is round-10/11 provenance: `VYPOCET-19` (2D, F-023, `supported`) and `VYPOCET-21` (4D, F-025, `partial`). Novelty: `verification/novelty/entropy-cluster.md`.*
