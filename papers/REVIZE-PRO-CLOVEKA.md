...
| **draft-04** — přechod typů na kauzálních množinách (v0.1) | Interní explorační draft; NENÍ připraven k odeslání. | Otevřená: proxy nejsou typy; ~~placeholder chyba `a_err=0.776`~~ (VYŘEŠENO 2026-06-06, viz níže); větší N v 4D potřeba | 3/3 proxy ukazuje III₁→II přechod ve 4D desce; selektivita (N^{3/4} funguje, fixed-fraction nefunguje) je nejsilnější argument | Referee napadne: „Konečná matice = vždy typ I; nic jste neměřili"; N^{3/4} je předpis, který jste sami vnutili | 12–20 hodin (re-run 4 calc.py + opravit statistiku + verifikace 15 arXiv ID) |
