...
| 1708.07445 | Mielczarek–Trześniewski (2018) | "Towards the map of quantum gravity": compares QG approaches (LQG, CDT, Hořava, AS, NCG, Quantum Graphity) incl. running spacetime dimension | The most directly comparable prior *survey*. See §3.1 for the boundary. |
...
The sharpest version of the referee attack is: *"This is the Calcagni program (1311.3340, 1304.2709, 1408.0199) repackaged. Calcagni already computes $d_s$ across approaches, already notes it is not universal, already knows it is probe/structure-sensitive in causal sets."* We accept that the overlap is high and state precisely where our line is:

- Calcagni and collaborators compute $d_s$ for each approach with the method appropriate to that approach (Seeley–DeWitt and saddle point for QFT/Stelle; combinatorial Laplacians for GFT/spin foam; fractional measures for multifractional spaces). Our value-add is a **single functional $P(\sigma)=\int d^Dk\,e^{-\sigma F(k)}$, one numerical engine, one $d_s$ definition**, validated against *all* of these published numbers simultaneously (12/12). The "map of quantum gravity" (Mielczarek–Trześniewski, 1708.07445) is a survey collation across approaches, not a unified-engine computation.
...
| Hořava–Lifshitz | heat kernel (anisotropic) | $z=2$; $D_{\rm space}=3$ | $\mathbf{5/2}$ | $4$ | REPRODUCE | 0902.3657 |
| Hořava–Lifshitz | heat kernel (anisotropic) | $z=3$; $D_{\rm space}=3$ | $\mathbf{2}$ | $4$ | REPRODUCE | 0902.3657 |
...
| **Causal sets** | **random walk on causal graph** | $z_{\rm eff}<1$ | $\mathbf{>D}$ (rises; num. $8$†) | $4$ | REPRODUCE (qual.) | 1311.2530 |
| Multifractional (Calcagni) | heat kernel (fractional measure) | $z_{\rm eff}=D/2$ | $\mathbf{2}$ | $4$ | REPRODUCE (compare) | 1304.2709 |

† **Illustrative value only, not from literature.** The numerical value $8=D+4$ is an *ad hoc* choice inserted to make the rising trend visible in plots and validation checks; Eichhorn–Mizera (1311.2530) publish no universal asymptotic constant. This row is qualitative only and makes no quantitative claim about the magnitude of the UV rise. See §6 limit 1 for the full caveat.

**Numerical agreement.** All 12 validation checks pass: GR UV $=4.0$; Hořava $z=2$ UV $=2.5$; Hořava $z=3$ UV $=2.0$; Stelle / AS / d'Alembertian / multifractional UV $=2.0$; all IR $=4.0$; random walk UV $=8.0>D$ (**illustrative value only**, not from Eichhorn–Mizera 1311.2530; see †-footnote above and §6 limit 1). The symbolic isotropic master returns $D/\gamma$.
...
`ds_flow.png` overlays every $d_s(\sigma)$ flow (x-axis $\log_{10}(1/\sigma)$: IR left, UV right). All curves start at $d_s=4$ in the IR and descend to their UV values of $2$ or $5/2$ — **except** the random-walk probe on the causal set, which alone *rises*, toward $8$ (an illustrative value, not from Eichhorn–Mizera 1311.2530; see §5.1† and §6 limit 1). The single rising curve is the visual statement of the whole note.
