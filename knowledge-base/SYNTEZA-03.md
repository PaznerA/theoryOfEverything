...
>
> **Forward-note (kolo 22, 2026-06-09 — F-040/F-041 přidány po sepsání této syntézy):** Compute-doporučení #1 z §5 (non-surogátní geometrický $\gamma_5$-gradovaný boost Dirac) bylo PROVEDENO. **F-040** (`confirmed-mixed-sharper-negative`): geometrický boost Dirac byl postaven, je well-posed na konečném causetu, a **vyřešil log-kompresní obstrukci F-036** (zákonný exponent $+1$ ne $0.72$, koeficient $O(2\pi)$ ne off $4.8\times$) — ALE absolutní Unruh $2\pi$ se stále neobnovuje ρ-invariantně (operátorová routa **driftuje** s $N$, CV 0.205); obstrukce se PŘESUNULA z ε-log-komprese na konečné-$N$ diskretizaci. **F-041** (`confirmed`): z téhož $\mathrm{Tr}\,e^{-\sigma D^2}$ heat-trace plyne $d_s=2$ (BD sonda) i Seeley-DeWittovy $a_k$ — ale diskrétní $a_4/a_0$ NEreprodukuje kontinuální $-18/11$ na plochém causetu (most NCG↔$d_s$ z §5 #2 částečně proveden, Wall-3 separace potvrzena z NCG strany). Verdikty linií 1.2/1.3 níže se NEMĚNÍ (zostřují se); §5 #1 přesunout z „lovit" na „proveden, ostřejší negativ"; §5 #2 z „odpoledne→sparse" na „částečně proveden". Plná aktualizace mapy v `papers/draft-06-discrete-program-limits/` a `reports/2026-06-09-velke-review.md`.
