...
> 🟢 **KOLO 22 — GEOMETRICKÝ DIRAC (UNRUH 2PI, ZED 2) + NCG<->D_S HEAT-KERNEL (F-040/041, 2026-06-09)** — Dvě výzkumné stopy + adversariální audit. **F-040 (T1, geometrický/gamma_5-gradovaný boostový Dirac, H6g-1, status: confirmed-mixed-sharper-negative):** pojmenovaný chybějící prvek F-036 POSTAVEN a WELL-POSED na konečném causetu: {D,Gamma5}=0 strojově nulové (Clifford), D hermitovský, Gamma5^2=I, +-párové chirální spektrum na 2.9e-15 -- první sudý spektrální triple s chirálním gradováním na causetu. Geometrický Dirac VYŘEŠÍ log-kompresní obstrukci F-036: exponent p_E=+1 (0.989, R2=1.000) ne surogatních 0.72, koeficient řádu 2pi ne off 4.8x. ALE absolutní 2pi se NEOBNOVUJE rho-invariantně: Route1 tautologická (w_boost=2pi*rho nese 2pi z definice), Route2 diagonála exaktně nulová, Route3 (operátorová spektrální routa K_op=(1/2){xi,p}) DRIFTUJE s N (4.747->5.647->7.707, CV=0.205). Obstrukce přesunuta z eps-log-komprese (F-036) na konečné-N diskretizaci prvořádového boostového generátoru -- ostřejší, lépe lokalizovaný negativ. Wall 2 NEPŘECHÁZÍ na pozitivní; hrana NCG<->semiklasika zůstává barely s ostřejší anotací. Korespondence: partial (sharper-negative). **F-041 (T2, NCG<->d_s heat-kernel, L3-1/L1-1, status: confirmed):** z téhož D^2-spektra (|sym(B)|, BD d'Alembertián) se na 2D causal diamondu extrahují KONZISTENTNĚ d_s(sigma) i Seeley-DeWittovy koeficienty a0,a2,a4. d_s=1.998±0.044 (N=2200, chyba 0.0024 < F-001 tol 0.06, plateau 1.83->2.25 dekád); Pauli-Jordanova sonda d_s=0.011 (F-001/F-002 probe-závislost zkonkretizována). Diskrétní a4/a0=-1.40±4.25 NEreprodukuje -18/11 (na plochém causetu a4=0 analyticky -- honest negativ). Hrana NCG<->spectral-dimension UPGRADOVÁNA barely->partially (shared-math, týž Tr e^{-sigma D^2}). Korespondence: partial (smíšený). **Audit:** oba verdikty potvrzeny; anti-cirkularita obstála (route1 správně označena tautologickou, route3 rho-invariance FALSE); F-040 status = confirmed-mixed, NE positive; F-041 = confirmed smíšený. **Hrany:** NCG<->spectral-dimension UPGRADE barely->partially (F-041); NCG<->semiclassical-gravity zůstává barely s F-040 ostřejší anotací (wall2_positive=False). Pipeline: fragment→consolidate→link-prediction (AUC 0.9063). **Testy: 365 passed, 31 skipped, 1 xfailed** (bez regrese; +1 skip = ncg-spectral-dimension nově v SLOW_CALCS). Web: python3 web/build.py. findings.json = **41 nálezů** (F-001..F-041, kontinuální).

> 🟢 **KOLO 21 — VELKÁ KONSOLIDACE PO DRUHÉM OBLOUKU (2026-06-09)** — Druhý výzkumný oblouk (kola 13–20, F-029…F-039) konsolidován; projekt přechází z numeriky do fáze **konsolidace/dokumentace**. **Stav:** 39 nálezů (F-001..F-039, kontinuální, 104/104 evidence na disku); 5 draftů + draft-06 negativ-letter (všechny s DRAFT bannerem, navzájem konzistentní, žádný neupgraduje status); **dvojí verifikace** — CAS 175/175 symbolických checků (7 WL skriptů, 38 vzorců: 24 verified + 14 already_validated, **Myrheim-Meyer RESOLVED** oprava /2) + numerická reprodukční coverage druhého oblouku (10/11 PASS, `ds-amol-convention` vědomě vyloučen); agent framework (3 agenti + adversariální audit); graf 298 hran (114 barely / 115 partially / 69 well), 626 uzlů / 2487 hran; SYNTEZA-03 uzavírací (tři „zdi": 4D area-zákon genuinně chybí F-031/037/038, surogátní Dirac = struktura ne teplota/metrika F-033/036, koeficient geometrický ne anomální F-039). **Integritní verdikt:** A (registry) CLEAN, B (drafty) issues-found, C (graf/dokumentace) DRIFT zaceleny. **Auto-fixy:** oprava stale F-033 korelace **0.098 → 0.319** (timing-truncace artefakt; verdikt no-match nezměněn) napříč fragmentem→consolidate→registry + 6 prose souborů + sjednocení F-033 polí ve findings.json; INDEX (3 rozbité odkazy, počty 31→36 výpočtů / 37→38 vzorců, +BRAINSTORM-06 +numerical-coverage report); `lib/toe` re-export kms_temperature/unruh_proper_law; REVIZE banner rozšířen o F-036/037/038/039 + F-034 superseder; ref-integrity BRAINSTORM-05 + draft-01 caveat. **Testy: 356 passed, 29 skipped, 1 xfailed** (bez regrese). Report: [`reports/2026-06-09-consolidation.md`](reports/2026-06-09-consolidation.md). **Roadmapa: konsolidace HOTOVA → compute režim → velké review.**

> 🟢 **KOLO 20 — CAS REVIZE VZORCŮ: TRIÁŽ + WOLFRAM VALIDACE B1–B4 (2026-06-08)** — Systémová triáž celého `core-data/formulas.json` (247 záznamů): 34 CAS-checkable, 158 definitional, 36 numerical, 19 already-validated. Wolfram Language validace 4 dávek: **33 verified, Myrheim-Meyer factor-2 OPRAVEN** (fragment jmenovatel 4→2, `resolved_blocker`). ✅ **`myrheim-meyer` RESOLVED:** jmenovatel v registru byl 4, správná publikovaná hodnota je 2 — f₀(d)=Γ(d+1)Γ(d/2)/(2Γ(3d/2)); d_MM≈2,38 reprodukovatelné pouze s /2. Oprava aplikována: `causal-sets.json` fragment 4→2 + consolidate.py; formula-coverage.json eviduje `resolved_blocker`, CAS lane 29/29 i 175/175 čistě. Infrastruktura: `run_all.py` rozšířen o B1, B2, B4 (celkem 7 WL skriptů); `verification/cas/formula-coverage.json`. **Testy (po konsolidaci kola 21): 356 passed, 29 skipped, 1 xfailed.**

> 🟢 **KOLO 19 — -18/11 overlaps (H-B EE-koeficient, H-E index-náboj, +6 hran / 2026-06-08)** — Tři výpočetní stopy + adversariální audit. **F-039 (T1 H-B, EE-koef vs anomálie, status: confirmed, negativní/diskriminační):** c_EE = 7.562 +/- 1.3% (CV) NEODPOVÍDÁ žádnému pre-registrovanému trace-anomálnímu racionálu; přímý skalární cíl c/(-a)=-3 míjí o 152 %, sekundární indexem chráněné NCG fermionové jádro -18/11 míjí o 362 %; nejbližší kandidáti jsou geometrické/kontrolní konstanty 8 (5.5 %, stále NAD prahem 5 %) a Bekenstein 4 (10.3 %). c_EE je GEOMETRICKÝ (kappa-cutoffem řízený) koeficient, NE konformně-anomální náboj. Anti-cirkularita ověřena. Korespondence: no-match-geometric. **T2 H-E (index-náboj diskrétní, status: documented-refuted):** H-E v přímé formě VYVRÁCENA; eta(iDelta)=0 na 48/48 bězích strukturálně (chybí chirální gradování gamma5), eta(D_K)~N^1.02 extenzivní (slope 1.019), n_rel~N^2.00 — žádný diskrétní proxy nereproduikuje N-nezávislý sudě-celočíselný Rohlinův náboj -2. VYPOCET-36 dokumentuje charakter gapu. F-040 NEPŘIDÁN (čistý negativní/analogický výsledek, ne finding). Korespondence: refuted. **+6 hran (T3):** spectral-action<->trace-anomaly (well), spectral-action<->central-charge (well), NCG<->trace-anomaly (partially), NCG<->spectral-dimension (barely), trace-anomaly<->EE (partially), a-theorem<->EE (well). Pipeline dodržen (fragmenty→consolidate→link-prediction). Audit: všechny 3 verdikty potvrzeny (T3 minor: trace-anomaly<->EE mohlo být barely, description poctivě odlišuje). Podmíněná hrana NCG<->EE (LOV #5) NEPŘIDÁNA — H-B test ji oslabuje. **Testy: 356 passed, 22 skipped, 1 xfailed.** findings.json = 39 nálezů (F-001..F-039). Link prediction: AUC 0.9057 (mírně nahoru), P@50 0.9975.

> 🟢 **KOLO 18 — T1 KMS/Unruh + T2 konformní 4D + T3 molekulová fluktuace (F-036/037/038 / 2026-06-08)** — Tři výpočetní stopy + adversariální audit. **F-036 (T1, NCG<->semiklasika KMS/Unruh, H6g-1, status: informovany-negativ-tautologie):** netautologický test selhal kvantitativně — zákonný exponent p_E=0,720 (BW +1 nečteno, deficit 28 %), koeficient 2*pi nejlepší routou 9,58 (off 52 %); rho-invariantní boostová diagonála (sklon 29,2, R2=0,969, CV 1,7 %) strukturálně solidní, kontroly selhávají (interval R2=0,038, shuffle R2=0,065). dS static patch potvrzuje (p_E=0,62, Gibbons-Hawkingova 2*pi nečtena). Korespondence: tautologie — most struktury, NE teploty; hrana NCG<->semiklasika zůstává barely. Referenční caveat: 1712.04227 chybně připsáno jako zdroj BW/Unruh-2pi (správně: bisognano1976duality/unruh1976notes). **F-037 (T2, konformní 4D dS area-zákon, H6g-2, status: supported):** konformní vazba xi=1/6 NEOBNOVI 4D dS area-zákon — R' drift rho^+0,386 identický pro xi=1/6 i xi=0 (CV 0,275 vs 0,276), S_full conf/massless = 1,0001; caveat (a) F-031 (xi-část) VYŘEŠEN; H6g-2 blokátor odstraněn (masivní SJ PSD na 1e-15). F-031 stojí silněji. Korespondence: negative. **F-038 (T3, molekulová fluktuace H6g-6, status: refuted-direction):** Var(N_mol)~rho^0,656, bootstrap CI95 [0,575, 0,745] vylučuje plochu rho^0.5 i objem rho^1.0; super-Poisson (Fano 3,72->5,30, CI68 vylučuje 0). Druhý nezávislý negativ k F-031. Korespondence: negative. Audit: všechny 3 verdikty potvrzeny (T1 minor: referenční mis-atribuce; T2 OK; T3 minor: záměna F-034/F-038 ve VYPOCET-34 ř.85); edge NCG<->semiklasika NEPROPAGOVÁNA (T1 tautologie dle auditu). **Testy: 356 passed, 22 skipped, 1 xfailed.** findings.json = 38 nálezů (F-001..F-038).

> 🟢 **KOLO 17 — H6g-6 MOLEKULOVÁ FLUKTUACE (ORDER-BY-DISORDER / VARIANCE-OSA / 2026-06-08)** — VYPOCET-34 (ds-molecule-fluctuation): nese 4D dS horizontovou entropii FLUKTUACE kodim-2 Dou-Sorkinova molekulového počtu, kde mean selhal (F-031)? Přes 5 hustot (ρ∈{120,240,480,960,1920}, N≤7677) × 200 seedů, čistě kombinatoricky (žádný eigh): mean <N_mol>~ρ^0.540 (plocha, F-031 potvrzen), ale **Var(N_mol)~ρ^0.656, bootstrap CI95 [0.575,0.745] VYLUČUJE plochu ρ^0.5 i objem ρ^1.0**; Var/A_proper driftuje (CV 0.66); počet je **super-Poisson** (Fano roste 3.7→5.3, slope ρ^0.116, CI68 vylučuje 0). **VERDIKT F-038: negative — 4D area-zákon NEPŘÍTOMNÝ na OBOU osách (mean i variance).** Druhý nezávislý negativ k F-031. Kontrast s F-035: tam Poisson (Fano=1, variance čistá), tady korelované molekuly (super-Poisson, variance nekóduje plochu). Nový lib primitiv `molecule_count_fluctuation` (+5 testů), toe v0.3.1. Bit-identická reprodukce (FULL_REPRO).

> 🟢 **KOLO 17 — HON NA VAZBY + REVIZE TEZÍ/PAPÍRŮ (DATA-AKTIVOVANÉ HRANY + PROPAGACE F-024..F-035 / 2026-06-08)** — 5 hran connections.json data-aktivováno nebo anotováno: idx 61 causal-sets<->NCG (barely, anotace F-033+F-034, metrická osa negativ + tepelná/KMS kvalitativní pozitiv); idx 289 von-neumann<->causal-sets (partially, doplněny F-029/F-031/F-032: proxy3 3/3, 2D area-zákon pub-strength, 4D area-zákon genuinně chybí); idx 59 causal-sets<->black-holes-information (partially, doplněn F-031 kontrast 2D-ano/4D-ne); idx 65 causal-sets<->cosmological-constant-fluctuation (partially, doplněny F-035 variancní/boost pozitiv + F-005 mean negativ — komplementární osy); idx 212 semiclassical-gravity<->causal-sets UPGRADE barely->partially (SJ-rotujici/superradiance linie F-009/013/017/018/030 — 5 nálezů, dosud nezaznamenaný největší datový svazek projektu). Draft revize: draft-01 v0.3 (B(a) spojitý F-030, opravena atribuce 2602.09796 Häfner-Klein); draft-02 v0.2 (graviton non-rescue F-014, Λ negativ F-020); draft-03 v0.2 (F-001/F-002 propagace); draft-04 v0.2 (F-027/F-028/F-029/F-031/F-032, proxy3 3/3, 2D ano/4D ne area-zákon). REVIZE-PRO-CLOVEKA.md datum+kola 12-16+checklisty. Link prediction: AUC 0.9034 (topologie nezměněna, žádní top-moveři), barely 114->113, partially 112->113. **Testy: 346 passed, 20 skipped, 1 xfailed.** Web: 121 stránek.
...
- **Vstup:** tři nezávislé auditní fasety — A (registry/integrita), B (drafty), C (graf/dokumentace/verifikace). Integritní verdikt: **A CLEAN, B issues-found, C DRIFT** (zaceleno).
- **Konsolidovaný stav:** 39 nálezů (F-001..F-039), 5 draftů + draft-06 negativ-letter, dvojí verifikace (CAS 175/175 + numerika 10/11 PASS), agent framework, graf 298 hran (114 barely / 115 partially / 69 well) / 626 uzlů / 2487 hran, SYNTEZA-03 uzavírací (tři „zdi" druhého oblouku).
- **Auto-fix stale F-033 korelace 0.098 → 0.319** (timing-truncace artefakt z `spectral-triple-modular`, odhalen numerickou coverage; verdikt no-match NEZMĚNĚN). Přes pipeline: editace fragmentu `core-data/fragments/causal-sets.json` (hrana idx 61) → `consolidate.py` → přegenerovány `connections.json` + `concept-graph.json` (diff přesně 1 řádek/registr, ostatních 5 artefaktů byte-identických). Sjednoceny pole `implications` + `noveltyStatus` ve `findings.json` F-033 (vnitřní rozpor odstraněn). Prose: `00-INDEX.md`, `VYPOCET-29`, `BRAINSTORM-05`, `BRAINSTORM-06`, `SYNTEZA-03`.
- **Auto-fix INDEX:** 3 rozbité odkazy (H04-entropy-cluster-reframe, ESEJ-03-gravitace-jako-stin, ESEJ-04-vstup-pozorovatele); počty „31→36 výpočtů" a „37→38 vzorců + Myrheim-Meyer resolved"; 2 nové položky (BRAINSTORM-06, report numerical-coverage); hlavička kolo 21.
- **Auto-fix registry/API:** `lib/toe/__init__.py` doplněn re-export `kms_temperature/KMSFit/unruh_proper_law/UnruhLawFit` (konzistence 4 primitiv spectraltriple na top-level).
- **Auto-fix drafty:** REVIZE banner rozšířen o F-036 (informovany-negativ-tautologie) / F-037 (supported) / F-038 (refuted-direction) / F-039 (confirmed) + poznámka, že F-034 je předchůdce konsolidovaný do F-036; ref-integrity BRAINSTORM-05 (Dafermos–Luk → Häfner & Klein; Fröb u 2501.09669, oba **stále neověřené**); draft-01 §5 dostal hlavičkový arXiv-caveat jako draft-04/06. **Žádné arXiv ID nevymyšleno; DRAFT bannery zachovány.**
- **Zbývá člověku (punch-list):** plný přepis REVIZE „čtyři → pět draftů" (§1 nadpis, §2.5 draft-06 checklist, §4.3 reprodukční příkazy); stale modul-docstring `spectraltriple.py` („two units" → 4); rozhodnutí o případné deduplikaci barely párů v connections.json; venue/konvenční-ref rozhodnutí draft-04/06.
- **Testy: 356 passed, 29 skipped, 1 xfailed** (bez regrese). Report: [`reports/2026-06-09-consolidation.md`](reports/2026-06-09-consolidation.md).
...
- **Dávka B1 — heat-kernel / perturbativní gravitace (8 vzorců):** `onshell-oneloop-counterterm` (1/120, 7/20 ✅), `goroff-sagnotti-counterterm` (209/2880 ✅), `gravity-power-counting` (D=(d−2)L+2 ✅), `quantum-potential` (41/(10π) ✅), `n8-critical-dimension-formula` (D_c=4+6/L ✅), `two-plus-epsilon-fp` (g*=3/38ε ✅), `n8-spin-content` (Binomial → {1,8,28,56,70} ✅), `stelle-propagator` (partial fraction ghost ✅).
- **Dávka B2 — kauzálně-množinová kombinatorika (6 vzorců):** `benincasa-dowker-action-4d` (✅), `discrete-dalembertian-4d` (✅), `myrheim-meyer` (**❌ MISMATCH BLOCKER**), `kr-count` (✅), `poisson-sprinkling` (✅), `number-volume` (✅).
- **Dávka B3 — CFT centrální náboje a entropie ČD (6 vzorců, skript cft-central-charges-bh-entropy.wl):** `strominger-vafa-entropy` (✅), `cardy-formula` (✅), `cardy-btz` (✅), `brown-henneaux` (✅), `mass-dimension` (✅), `critical-dimension-anomaly` (✅).
- **Dávka B4 — swampland / inflace (7 vzorců):** `slow-roll-spectral-index` (✅), `wgc-kk-rate` (✅), `universal-pattern-formula` (✅), `species-scale` (✅), `species-scale-formula` (✅), `w-infinity-algebra` (✅), `bcj-jacobi` (✅).
- **⚠️ BLOCKER `myrheim-meyer`:** Registr má f₀(d)=Γ(d+1)Γ(d/2)/(4Γ(3d/2)), správná publikovaná hodnota je jmenovatel **2**, nikoli 4. Tři nezávislé kotvy: f₀(2)=1/2 (2D Minkowski), f₀(4)=1/10 (4D), inverze f₀=3/8 (KR) → d_MM≈2,38 — vše shodné pouze s /2. S /4 vychází d≈1,44 (nesedí nic). **Oprava:** `core-data/fragments/causal-sets.json`, pole `myrheim-meyer.latex`, jmenovatel 4→2; poté `workflows/consolidate.py`. Zdroj: Meyer 1988.
- **Infrastruktura:** `run_all.py` rozšířen o B1 (`heat-kernel-perturbative-gravity-counterterms.wl`), B2 (`causal-set-combinatorial-operators.wl`), B4 (`b4-swampland-inflation-scaling.wl`) — celkem 7 WL skriptů; syntakticky ověřeno. Nový `verification/cas/formula-coverage.json` (stav 37 vzorců, `mismatch_blocker` v meta). `verification/cas/README.md` rozšířeno o B1/B2/B4/formula-coverage sekce.
- **Report:** [`reports/2026-06-08-cas-formula-revision.md`](reports/2026-06-08-cas-formula-revision.md).
- **`00-INDEX.md`** aktualizován: nové položky B1/B2/B4 WL skripty + formula-coverage.json + report kola 20.

### 2026-06-08 (kolo 19 — -18/11 overlaps: H-B EE-koeficient vs anomálie + H-E index-náboj diskrétní + +6 hran; F-039; adversariální audit)

- **VYPOCET-35 — H-B EE-koeficient vs trace-anomálie (F-039, confirmed, negativní/diskriminační):**
  - c_EE = 7.5623 = 1/R_full, R_full = 0.13224, CV 1.26 % (z F-029 dS path). 10 kandidátů pre-registrováno (anti-cirkularita ověřena: candidates zapsáni PŘED čtením c_EE). 6 kanálů × 10 kandidátů = 60 srovnání; matches_within_5pct=[]; n_matches=0.
  - Přímý skalární cíl c/(-a)=-3: residual 152.08 %. Sekundární indexem chráněné fermionové jádro -18/11=1.636: residual 362.14 %. Nejbližší: kontrolní konstanta 8 (5.47%, NAD prahem 5%) a Bekenstein 4 (10.26%, 1/c_log kanál).
  - Čerstvý 2D diamant: S_trunc = 0.227*ln(N) - 0.045 (r2=0.98), c_log=0.227, 1/c_log=4.41 -- žádná shoda s anomálním racionálem. Runtime 233 s.
  - **Verdikt F-039:** confirmed, no-match-geometric — c_EE je GEOMETRICKÝ (kappa-cutoff řízený) koeficient, NE konformně-anomální náboj. Indexem-chráněné NCG jádro -18/11 se přes tento kanál NEDOTÝKÁ entanglement-entropy linie B. Scope: 2D, finite-N, massless scalar.
- **VYPOCET-36 — H-E index-náboj diskrétní (dokumentovaný negativní výsledek; F-040 NEPŘIDÁN):**
  - Probe A eta(D_K) surogátní Dirac: slope 1.019 (eta~N^1.02, eta/N~0.21 stabilní), CV=0.42 — extenzivní, NE N-nezávislý sudý náboj. Probe B eta(iDelta) Pauli-Jordan: 0.000 na 48/48 bězích (CV=0) — strukturálně nula (±-párované spektrum, absence gamma5 chirálního gradování). Probe C slopes: n_rel~N^2.00, n_link~N^1.20, n_mol~N^0.32 — všechny extenzivní.
  - **Verdikt VYPOCET-36:** H-E v přímé formě VYVRÁCENA. Indexová ochrana -18/11 je vlastnost HLADKÉHO gradovaného spinového sektoru, ne Lorentzovského causal setu. Skutečný most by vyžadoval euklidizaci, gamma5-gradování, saturující invariant. **F-040 NEPŘIDÁN** (čistý negativní/analogický výsledek, ne nový finding).
- **T3 HRANY (+6 hran do connections.json, pipeline: fragmenty → consolidate → link-prediction):**
  - spectral-action<->trace-anomaly (shared-math, well): F-003/F-014 + hep-th/9606001/hep-th/0306138.
  - spectral-action<->central-charge (shared-math, well): táž evidence.
  - noncommutative-geometry<->trace-anomaly (shared-math, partially): pilíř-level umbrella.
  - noncommutative-geometry<->spectral-dimension (shared-math, barely): link-pred score 0.896, heat-kernel shared.
  - trace-anomaly<->entanglement-entropy (shared-math, partially): CHM + Solodukhin kontinuální CFT.
  - a-theorem<->entanglement-entropy (shared-math, well): CHM a-věta přes EE monotonii.
  - UMÍSTĚNÍ: 4 NCG hrany → noncommutative-geometry.json; 2 anomálie/EE hrany → semiclassical-gravity.json.
  - PODMÍNĚNÁ HRANA NCG<->EE (LOV #5): NEPŘIDÁNA — H-B test ji oslabuje (no-match-geometric).
  - Link prediction po aktualizaci: AUC 0.9057 (bylo 0.9034), P@50 0.9975, edges 1638 (bylo 1632, +6).
- **ADVERSARIÁLNÍ AUDIT:** T1 OK (verdikt confirmed, anti-cirkularita ověřena, c_EE=F-029); T2 OK (H-E refutace čistá, gamma5-gap explicitní, extenzivní škálování ověřeno); T3 minor (trace-anomaly<->EE mohlo být barely místo partially — description poctivě odlišuje, není zavádějící). Podmíněná NCG<->EE hrana správně podržena.
- **Testy: 356 passed, 22 skipped, 1 xfailed v 157 s** (beze změny struktury).
- **findings.json = 39 nálezů** (F-039: H-B EE-koeficient vs anomálie, confirmed/negativní; F-040 nepřidán).

### 2026-06-08 (kolo 18 — T1 KMS/Unruh + T2 konformní 4D + T3 molekulová fluktuace; F-036/037/038; adversariální audit)

- **VYPOCET-32 — H6g-1(b) KMS/Unruh netautologický test (F-036, informovany-negativ-tautologie):**
  - Zákonný exponent p_E=0,720+-0,040 (BW +1 nečteno, deficit 28 %); boost slope 29,20+-0,87 (R2=0,969, CV 1,7 %, rho-invariantní); best 2*pi routa boost_quantum=9,58 (off 52 %); unruh_ratio=0,790 (off 87 %). dS static patch: p_E=0,62, Gibbons-Hawkingova 2*pi=2,55. Kontroly selhávají (interval R2=0,038, shuffle R2=0,065 << 0,97). N in {600,1000,1400}, 5 seedů každý (15 Rindler + 15 dS), wall 53,7 s.
  - **Verdikt F-036:** informovany-negativ-tautologie — most struktury (boostová geometrie), NE teploty; hrana NCG<->semiklasika zůstává barely, annotována informovaným negativem. Edge NEPROPAGOVÁNA (audit: T1 tautologie).
- **VYPOCET-33 — H6g-2 konformní 4D dS area-zákon (F-037, supported / negativ F-031 posiluje):**
  - R' drift +0,386 identický pro xi=1/6 i xi=0; S_full conf/massless = 1,0001; masivní SJ PSD (pairing 8,4e-15, Wightman PSD 1e-15); rho in {120,240,480}, 4 seedy, wall 1027 s.
  - **Verdikt F-037:** supported — 4D area-zákon absence je robustní fyzika; caveat (a) F-031 (xi-část) VYŘEŠEN; H6g-2 blokátor odstraněn. Korespondence: negative.
- **VYPOCET-34 — H6g-6 molekulová fluktuace (F-038, refuted-direction):**
  - Var(N_mol)~rho^0,656, CI95 [0,575, 0,745] vylučuje rho^0.5 i rho^1.0; super-Poisson (Fano 3,72->5,30, slope rho^0,115, CI68 vylučuje 0); 5 hustot x 200 seedů, wall ~515 s. Mean F-031 potvrzen.
  - **Verdikt F-038:** refuted-direction — druhý nezávislý negativ k F-031. Korespondence: negative.
- **ADVERSARIÁLNÍ AUDIT:** T1 minor (referenční mis-atribuce 1712.04227, BW/Unruh-2pi odkazovat na bisognano1976duality/unruh1976notes); T2 OK (žádný blokátor); T3 minor (záměna F-034/F-038 ve VYPOCET-34 ř.85, fyzika nezměněna). Všechny verdikty potvrzeny.
- **Testy: 356 passed, 22 skipped, 1 xfailed v 155,72 s** (beze změny struktury).
- **findings.json = 38 nálezů** (F-001..F-038).

### 2026-06-08 (kolo 17 — hon na data-aktivované vazby + revize tezí/papírů; propagace F-024..F-035)

- **HON NA VAZBY (hunt) — 5 hran connections.json:**
  - **idx 61 causal-sets<->NCG (barely, bez upgrade):** anotace potvrzena (F-033+F-034); dvě testované osy: (A) metrická osa — Connesova vzdálenost NO-MATCH (R^2=0.01), (B) tepelná/KMS osa — SJ modulární tok = pravý jednoteplotní KMS tok (beta_KMS=1 strojová přesnost, rho-invariantní boost R2=0.953); absolutní Unruh 2pi NEobnovena (ratio=0.786). Informovaný negativ na metrické ose, kvalitativní pozitiv na tepelné ose — hrana zůstává barely, ale jde o OTESTOVANOU zónu.
  - **idx 289 von-neumann<->causal-sets (partially, bez upgrade):** doplněny F-029/F-031/F-032; proxy3 upgradována ze 2/3 na 3/3 při >=30 seedech (t=3.25); 2D area-zákon robustní (R=0.130, CV=3%, drift≈0, pub-strength); 4D area-zákon genuinně nepřítomný (S_full~rho^1 vs A_codim2~rho^0.5, poměr driftuje); II1/II_inf content-saturace se čistě zveda do 4D.
  - **idx 59 causal-sets<->black-holes-information (partially, bez upgrade):** doplněn F-031 — 2D dS SJ entropy cap A/4-like existuje (c~7.57–7.70, F-028+F-029), ve 4D genuinně nepřítomný.
  - **idx 65 causal-sets<->cosmological-constant-fluctuation (partially, bez upgrade):** doplněny dvě komplementární osy — mean-prefaktor NEGATIV (F-005: kappa mismatch 139.6x), variancní/boost POZITIV (F-035: Fano=0.9986, delta_Lambda~V^{-0.484}, boost-invariantní, mřížkový kontrast 5.13x); žádný z pozitivů nevzkříšuje mean-prefaktor.
  - **idx 212 semiclassical-gravity<->causal-sets (UPGRADE barely->partially):** SJ-rotujici/superradiance linie (F-009/F-013/F-017/F-018/F-030) — 5 nálezů, dosud nezaznamenaná v žádné hraně, nejhustší datový svazek projektu. Klíčové: B(a) je SPOJITÁ klesající funkce strhávání (6.10->2.54, dB/da=-2.20, z=-33.6); B=D-1=3 zamítnut (chi2/dof~350); BTZ pod Kerr křivkou = evidence role asymptotiky; superradiantní váha řízena lokální ZAMO Omega(r), NE ergosférou jako prahem; eigenvektor-rotace ~45 stupnů.
- **REVIZE DRAFTU (revise) — 4 drafty aktualizovány:**
  - **draft-01 v0.3:** §4.2 B(a) spojitý (6.10->2.54, dB/da=-2.20, konstantní B=3 zamítnut), opravena chybná atribuce arXiv:2602.09796 (bylo Dafermos-Luk, správně Häfner-Klein), připojeny F-013/F-009.
  - **draft-02 v0.2:** §4 graviton non-rescue (F-014, -18/11 = striktní fermionový diskriminátor), §5 poctivý negativ F-020 (žádná sesterská identita pro Λ).
  - **draft-03 v0.2:** propagace F-001 (master limit D/gamma), F-002 (probe-dependence).
  - **draft-04 v0.2:** propagace F-019/F-016/F-024/F-026/F-027/F-028/F-029/F-031/F-032; proxy3 2/3->3/3 (F-032, seed-count efekt); 2D area-zákon rozšířen (F-029), 4D area-zákon poctivý negativ (F-031); outlook A/4 přepsán.
  - **REVIZE-PRO-CLOVEKA.md:** datum 2026-06-08, kola 12-16 + agent-framework blok, per-finding checklisty F-027..F-032+F-016/F-019/F-024/F-026.
- **LINK PREDICTION refresh:** topologie nezměněna (0 nových/odebraných hran), AUC 0.9034 (beze změny), P@50=0.9975, nodes/edges 626/1632. Jediný pohyb: barely 114->113, partially 112->113 (upgrade idx 212). Per-komponentní AUC 0.9297->0.9295 (zaokrouhlovací šum). Top-moveři: žádní — predikce pracuje nad topologií, slepá vůči textu/ratingu.
- **Testy: 346 passed, 20 skipped, 1 xfailed v 153.9 s** (beze změny oproti kolu 16 — žádné nové výpočty).
- **Web přebuildován:** 121 stránek (115 markdown + 6 generovaných, +10 oproti kolu 16).
