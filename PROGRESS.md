...
> 🟢 **KOLO 20 — CAS REVIZE VZORCŮ: TRIÁŽ + WOLFRAM VALIDACE B1–B4 (2026-06-08)** — Systémová triáž celého `core-data/formulas.json` (247 záznamů): 34 CAS-checkable, 158 definitional, 36 numerical, 19 already-validated. Wolfram Language validace 4 dávek: **33 verified, 1 mismatch (BLOCKER)**. ⚠️ **BLOCKER `myrheim-meyer`:** jmenovatel v registru je 4, správná publikovaná hodnota je 2 — f₀(d)=Γ(d+1)Γ(d/2)/(2Γ(3d/2)); d_MM≈2,38 reprodukovatelné pouze s /2. Oprava: `causal-sets.json` fragment jmenovatel 4→2 + consolidate.py. Infrastruktura: `run_all.py` rozšířen o B1, B2, B4 (celkem 7 WL skriptů); nový `verification/cas/formula-coverage.json`. Testy: viz aktuální stav.
